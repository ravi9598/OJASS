...
public class Armageddon extends Fragment {
    List<String> subEvents = new ArrayList<>();
    @Nullable
    @Override
    public View onCreateView(@NonNull LayoutInflater inflater, @Nullable ViewGroup container, @Nullable Bundle savedInstanceState) {
        View view = inflater.inflate(R.layout.armageddon,container,false);
    ...
    List<String> getSubEvents(){
        subEvents.clear();
        for(int i = 0;i<EventList[11].length;i++){
            subEvents.add(EventList[11][i]);
        }
        return subEvents;
    }
}
